...
def create_poll(chat_id: int, question: str, creator_id: int, options: list) -> int:
    """
    options: প্রতিটা আইটেম হয় শুধু একটা নাম (str), অথবা (নাম, manual_votes) —
    দ্বিতীয়টা দিলে সেই অপশন প্রি-সেট ভোট সংখ্যা নিয়ে শুরু হবে।
    """
    normalized = [
        opt if isinstance(opt, tuple) else (opt, 0)
        for opt in options
    ]
    with _lock, _connect() as conn:
        cur = conn.execute(
        ...
        conn.executemany(
            "INSERT INTO poll_options (poll_id, option_text, position, manual_votes) VALUES (?, ?, ?, ?)",
            [(poll_id, text, i, manual_votes) for i, (text, manual_votes) in enumerate(normalized)],
        )
    return poll_id
