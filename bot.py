...
def parse_option_input(raw_text: str):
    """
    অপশনের নাম পার্স করে — "নাম" বা "নাম,সংখ্যা" (যেমন "Mehedi,10") দুই ফরম্যাটই
    সাপোর্ট করে। নতুন পোল বানানোর সময় এবং এডিট মেনু থেকে অপশন যোগ করার সময়,
    দুই জায়গাতেই এই একই ফাংশন ব্যবহার হয়, যাতে দুই জায়গায় একই আচরণ থাকে।
    Returns: (option_name, manual_votes) অথবা None (ভুল ফরম্যাট হলে, যেমন
    কমার পরে সংখ্যা ছাড়া অন্য কিছু থাকলে)।
    """
    ...
    if "," in raw_text:
        ...
        if not count_part.isdigit() or not name_part:
            return None
        return name_part, int(count_part)
    return raw_text, 0


async def send_admin_dm(context: ContextTypes.DEFAULT_TYPE, user_id: int, **kwargs) -> bool:
    """
    এডমিনের DM-এ মেসেজ পাঠায় (এডিট মেনু, অপশন-যোগ প্রম্পট, ভোট-বাদ-দেওয়ার লিস্ট —
    এসব যেন চ্যানেল/গ্রুপে সবাই দেখতে না পায়, শুধু এডমিন নিজেই DM-এ দেখবে)।
    ব্যর্থ হলে (এডমিন বটকে প্রাইভেটে /start করেনি) False রিটার্ন করে।
    """
    try:
        await context.bot.send_message(chat_id=user_id, **kwargs)
        return True
    except Exception as e:
        logger.info("admin DM failed for user %s: %s", user_id, e)
        return False


DM_START_HINT = (
    "❗ আগে বটকে প্রাইভেটে /start করুন (বটের প্রোফাইলে গিয়ে Start চাপুন), "
    "তারপর আবার চেষ্টা করুন — এই মেনুটা এখন থেকে চ্যানেলে না দেখিয়ে আপনার DM-এ পাঠানো হবে।"
)
...
async def ask_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "কোনো অপশন আগে থেকেই একটা ভোট সংখ্যা নিয়ে শুরু করাতে চাইলে নাম,সংখ্যা লিখুন "
        "(যেমন: Mehedi,10) — তাহলে সেটা ১০ ভোট নিয়ে শুরু হবে এবং তারপর থেকে আসল ভোট "
        "যোগ হতে থাকবে।\n"
...
async def add_option(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parsed = parse_option_input(update.message.text)
    if parsed is None:
        await update.message.reply_text(
            "❌ ফরম্যাট ঠিক নেই। শুধু নাম লিখুন (যেমন Mehedi), অথবা আগে থেকে ভোট "
            "যোগ করে দিতে চাইলে নাম,সংখ্যা লিখুন (যেমন Mehedi,10)। আবার চেষ্টা করুন।"
        )
        return ASK_OPTIONS

    option_name, manual_votes = parsed
    options.append((option_name, manual_votes))

    confirm_line = (
        f"যোগ হয়েছে: {option_name} (শুরুতেই {manual_votes} ভোট)"
        if manual_votes
        else f"যোগ হয়েছে: {option_name}"
    )
    await update.message.reply_text(
        f"{confirm_line}\n\nমোট অপশন: {len(options)}\n\n"
        "আরও অপশন লিখুন (নাম অথবা নাম,সংখ্যা), অথবা শেষ হলে নিচের বাটনে চাপুন।",
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("✅ শেষ করুন", callback_data="options_done")],
                [InlineKeyboardButton("❌ বাতিল", callback_data="cancel")],
...
async def finish_options(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    poll_id = db.create_poll(chat_id, question, creator_id, options)  # options: [(name, manual_votes), ...]
...
async def handle_stopask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not sent:
        await query.answer(DM_START_HINT, show_alert=True)
...
async def handle_editmenu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sent = await send_admin_dm(
        ...
        text=f"✏️ \"{question}\" — এডিট মেনু। কী করতে চান?",
        reply_markup=InlineKeyboardMarkup(
            [
...
async def handle_addopt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sent = await send_admin_dm(
        ...
        text=(
            ...
            "বাতিল করতে /cancel লিখুন। (এই মেসেজের রিপ্লাইও এখানে DM-এই দিন।)"
        ),
        parse_mode=ParseMode.HTML,
    )
    if not sent:
...
async def handle_awaiting_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if parsed is None:
        await update.message.reply_text(
            "❌ ফরম্যাট ঠিক নেই। শুধু নাম লিখুন, অথবা নাম,সংখ্যা লিখুন (যেমন Mehedi,10)। "
...
async def handle_kickmenu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not voters:
        sent = await send_admin_dm(
            context, update.effective_user.id, text="এখনো কেউ (আসল ভোট দিয়ে) ভোট দেয়নি।"
        )
        if not sent:
            await query.answer(DM_START_HINT, show_alert=True)
        return
    ...
    sent = await send_admin_dm(
        context,
        update.effective_user.id,
        text="কাকে ভোট থেকে বাদ দিতে চান? (তার ভোট বাতিল হয়ে যাবে)",
